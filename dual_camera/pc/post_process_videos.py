...
def create_synchronized_video(cam0_path, cam1_path, output_path, layout='vertical', 
                            ...
                            cam0_fov=None, cam1_fov=None, cam0_center_x=None, cam0_center_y=None,
                            cam1_center_x=None, cam1_center_y=None):
    """
    ...
    try:
        ...
        # Apply FOV cropping to cam0
        cam0_crop_filter = ""
        if cam0_fov is not None:
            crop_params = calculate_fov_crop(cam0_rotated_width, cam0_rotated_height, 
                                           cam0_fov, cam0_center_x, cam0_center_y)
            if crop_params:
                ...
                print(f"Applied FOV crop to cam0: {crop_width}x{crop_height} at ({crop_x},{crop_y})")
            else:
                cam0_crop_filter = "[v0_rot]copy[v0_crop];"
        else:
            cam0_crop_filter = "[v0_rot]copy[v0_crop];"
        
        # Apply FOV cropping to cam1
        cam1_crop_filter = ""
        if cam1_fov is not None:
            crop_params = calculate_fov_crop(cam1_rotated_width, cam1_rotated_height, 
                                           cam1_fov, cam1_center_x, cam1_center_y)
            if crop_params:
                crop_width, crop_height, crop_x, crop_y = crop_params
                ...
                print(f"Applied FOV crop to cam1: {crop_width}x{crop_height} at ({crop_x},{crop_y})")
            else:
        ...
        if layout == 'vertical':
            ...
            # Always re-encode when using filter_complex
            print("Using fast mode (re-encoding)...")
            scale_filter = f"[v0_crop]scale={total_width}:{cam0_rotated_height}[v0];[v1_crop]scale={total_width}:{cam1_rotated_height}[v1]"
            ...
            print("Using fast mode (re-encoding)...")
            scale_filter = f"[v0_crop]scale={cam0_rotated_width}:{total_height}[v0];[v1_crop]scale={cam1_rotated_width}:{total_height}[v1]"
    ...
    class VideoSyncGUI:
        ...
        def process_videos(self):
            try:
                ...
                # Enforce same frame count using cam0's count
                for out in [out0, out1]:
                    temp_out = out + ".sync.mp4"
                    trim_cmd = [
                        'ffmpeg', '-y', '-i', out,
                        '-frames:v', str(frame_count0),
                        '-c:v', 'libx264', '-preset', 'ultrafast', '-crf', '23',
                        temp_out
                    ]
                ...
                subprocess.run(trim_cmd, check=True)
                # Extract cam0 and cam1 from the trimmed merged video
                width, height, *_ = get_video_info(trimmed_output)
                if layout == 'vertical':
                    # cam0: top half, cam1: bottom half
                    crop0 = f"crop={width}:{height//2}:0:0"
                    crop1 = f"crop={width}:{height//2}:0:{height//2}"
                else:
                    # horizontal: cam0 left, cam1 right
                    crop0 = f"crop={width//2}:{height}:0:0"
                    crop1 = f"crop={width//2}:{height}:{width//2}:0"
                # Overwrite cam0_trimmed.mp4 and cam1_trimmed.mp4
                crop_cmd0 = [
                    'ffmpeg', '-y', '-i', trimmed_output,
                    '-filter:v', crop0, '-c:a', 'copy', out0
                ]
                crop_cmd1 = [
                    'ffmpeg', '-y', '-i', trimmed_output,
                    '-filter:v', crop1, '-c:a', 'copy', out1
                ]
                subprocess.run(crop_cmd0, check=True)
                subprocess.run(crop_cmd1, check=True)
                messagebox.showinfo("Done", f"Trimmed and concatenated video saved as {trimmed_output}\ncam0/cam1 trimmed parts updated.")
            except Exception as e:
